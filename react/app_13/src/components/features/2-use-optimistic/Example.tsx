...
async function addTodoAPI(text: string): Promise<Todo> {
  await delay(5000);
  if (text === "reject") throw new Error();
  return {
    id: Date.now(),
    text,
    completed: false,
...
function OptimisticTodoList() {
  const [todos, setTodos] = useState<Todo[]>([
    { id: 1, text: "Learn React 19", completed: true, isPending: false },
    { id: 2, text: "Build awesome apps", completed: false, isPending: false },
  ]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      try {
        ...
        setTodos([...todos, { ...newTodo, isPending: false }]);
      } catch (error) {
